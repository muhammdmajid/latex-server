...
const extractZip = (zipFilePath: string, destDir: string): Promise<void> => {
  return new Promise((resolve, reject) => {
    try {
      const zip = new AdmZip(zipFilePath);
      zip.extractAllTo(destDir, true);
      logger.info(`Successfully extracted .zip file: ${zipFilePath} to ${destDir}`);
      resolve();
    } catch (error: unknown) {
...
const extractTarGz = (tarFilePath: string, destDir: string): Promise<void> => {
  return new Promise((resolve, reject) => {
    try {
      tar.x({ file: tarFilePath, cwd: destDir });
      logger.info(`Successfully extracted .tar file: ${tarFilePath} to ${destDir}`);
      resolve();
    } catch (error: unknown) {
      if (error instanceof Error) {
